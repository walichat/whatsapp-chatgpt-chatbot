...
const API_URL = process.env.API_URL || 'https://api.wali.chat/v1'

// Function to send a message using the WaliChat API
...
// Find an active WhatsApp device connected to the WaliChat API
export async function loadDevice () {
  ...
  if (config.device && !config.device.includes(' ')) {
    if (/^[a-f0-9]{24}$/i.test(config.device) === false) {
      return exit('Invalid WhatsApp device ID: must be 24 characers hexadecimal value. Get the device ID here: https://app.wali.chat/number')
    }
