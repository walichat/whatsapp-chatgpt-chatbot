...
async function main () {
  ...
  if (!config.apiKey || config.apiKey.length < 60) {
    return exit('Please sign up in WaliChat and obtain your API key:\nhttps://app.wali.chat/apikeys')
  }
  ...
  // Find a WhatsApp number connected to the WaliChat API
  const device = await actions.loadDevice()
  if (!device) {
    return exit('No active WhatsApp numbers in your account. Please connect a WhatsApp number in your WaliChat account:\nhttps://app.wali.chat/create')
  }
  if (device.session.status !== 'online') {
    return exit(`WhatsApp number (${device.alias}) is not online. Please make sure the WhatsApp number in your WaliChat account is properly connected:\nhttps://app.wali.chat/${device.id}/scan`)
  }
  if (device.billing.subscription.product !== 'io') {
    return exit(`WhatsApp number plan (${device.alias}) does not support inbound messages. Please upgrade the plan here:\nhttps://app.wali.chat/${device.id}/plan?product=io`)
  }
